...
/**
 * A specialized hook for the WriteMeAStory BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - input: string
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.Story
 * - **Streaming Partial:** Story
 * - **Streaming Final:** types.Story
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useWriteMeAStory({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useWriteMeAStory({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useWriteMeAStory(props: HookInput<'WriteMeAStory', { stream: false }>): HookOutput<'WriteMeAStory', { stream: false }>
export function useWriteMeAStory(props?: HookInput<'WriteMeAStory', { stream?: true }>): HookOutput<'WriteMeAStory', { stream: true }>
export function useWriteMeAStory(
  props: HookInput<'WriteMeAStory', { stream?: boolean }> = {},
): HookOutput<'WriteMeAStory', { stream: true }> | HookOutput<'WriteMeAStory', { stream: false }> {
  let action: ServerAction = Actions.WriteMeAStory;
  if (isStreamingProps(props)) {
    action = StreamingActions.WriteMeAStory;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'WriteMeAStory', { stream: false }>)
  }
}